...
template< typename RegisterContainerType, typename AddressType, typename ValueType, bool IsBigEndian = true, int Min = 0, int Max = 0, template< class, bool > class ReadWritePolicy = ReadWrite >
class GenericRegister : public AbstractRegisterBase
{
    ...
    public:
		...
		//Typedef For Registration Function That Will Store AbstractRegisterBase* pointer in relation to the address
		typedef int (*regRegistrationFcn)( uint32_t addr, AbstractRegisterBase* ptr );

		//Constructor Needs Address and Initial Value
		GenericRegister( uint32_t masterDeviceAddress, AddressType address, BaseValueType value, RegisterContainerType* regCont, regRegistrationFcn registrationFcn ) : 
				masterAddr_(masterDeviceAddress),
				addr_(address), 
				val_( value ) { 

			(regCont->*registrationFcn)( addr_, this );
		};
